...
import pp202402.assign2a.Assignment2A.{lookup, insert, optimize}
import pp202402.assign2a.{IOption, BST, Expr}
...
class TestSuite extends munit.FunSuite {
  ...
  test("problem 1 - Int") {
    ...
    assertEquals(
      lookup(
        Node(2, "a", Leaf, Node(3, "b", Leaf, Leaf)),
        cmpInt,
        3
      ),
      ISome("b")
    ...
    assertEquals(
      ...
      Node(4, "c", Node(2, "b", Leaf, Leaf), Node(6, "a", Leaf, Leaf))
    )
  ...
  test("problem 1 - String") {
    ...
    assertEquals(
      lookup(
        Node("ab", 3, Leaf, Node("abcd", 7, Leaf, Leaf)),
        cmpStr,
        "abcd"
      ),
      ISome(7)
    ...
    assertEquals(
      ...
      Node("ad", 3, Node("a", 2, Leaf, Leaf), Node("asdf", 1, Leaf, Leaf))
    )
    val t1 =
      List(("a", 1), ("bad", 2), ("af", 3), ("czq", 4), ("bes", 5), ("cs", 6))
  ...
  test("problem 2 - complex expression optimization") {
    ...
    // ((1 * x) + (3 - 3)) * (y / 1) => x * y
    val expr4 = Mul(
      Add(Mul(Num(1), Var("x")), Sub(Num(3), Num(3))),
      Div(Var("y"), Num(1))
    )
    ...
    // ((0 * x) + (5 - 3)) * (0 + 2) => 4
    val expr6 =
      Mul(Add(Mul(Num(0), Var("x")), Sub(Num(5), Num(3))), Add(Num(0), Num(2)))
    assertEquals(optimize(expr6), Num(4))

    // Complex expression with division: (1 * (x + 0)) / (y * (3 - 3)) => Division by zero exception
    val expr7 = Div(
      Mul(Num(1), Add(Var("x"), Num(0))),
      Mul(Var("y"), Sub(Num(3), Num(3)))
    )
