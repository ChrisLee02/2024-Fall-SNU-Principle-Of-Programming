...
import pp202402.assign1.Assignment1.*
...
class TestSuite extends munit.FunSuite {
  test("problem 1-1") {
    ...
    assertEquals(iteration((x) => if x % 2 == 0 then 4 * x + 1 else 2 * (x / 7), 50, 8), 8L)
  }
  ...
  test("problem 2") {
    assertEquals(combination(1, 2), 0L)
    assertEquals(combination(1, 3), 0L)
    assertEquals(combination(3, 3), 1L)
  ...
  test("problem 3-2") {
    ...
    assertEquals(nthTwinPrime(2), 5L) // 두 번째 쌍둥이 소수: (5, 7)
    assertEquals(nthTwinPrime(3), 11L) // 세 번째 쌍둥이 소수: (11, 13)
    assertEquals(nthTwinPrime(4), 17L) // 네 번째 쌍둥이 소수: (17, 19)
