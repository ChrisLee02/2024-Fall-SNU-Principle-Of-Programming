...
impl Expr {
    fn eval_env<'a, 'b>(&'a self, env_rc: &'b Rc<Env<'a>>) -> Val<'a> {
        fn eval_arith<'a, 'b>(
            ...
            let e1_eval = e1.eval_env(env_rc);
            let e2_eval = e2.eval_env(env_rc);
        ...
        fn eval_if<'a, 'b>(
            ...
            let cond_result = e_cond.eval_env(env_rc);
            match cond_result {
                VNum(n) => {
                    if n == 0 {
        ...
        fn eval_name<'a, 'b>(x: &'static str, env_rc: &'b Rc<Env<'a>>) -> Val<'a> {
            match &env_rc.entry {
                ...
                EEFuns(list) => {
                    for (name, func) in list.iter() {
                        if *name == x {
                            return VClo(*func, env_rc.clone());
                        }
                    }
                }
            }
        ...
        fn eval_app<'a, 'b>(f: &'a Expr, arg: &'a Expr, env_rc: &'b Rc<Env<'a>>) -> Val<'a> {
            let f_eval = f.eval_env(env_rc);
            let arg_eval = arg.eval_env(env_rc);

            match f_eval {
                VNum(_) => panic!("error: function not found"),
                VClo(function, fun_rc) => {
                    let (var_name, func_body) = function;
                    let new_env =
                        Rc::new(Env::new(Some(fun_rc.clone()), EEVal(&var_name, arg_eval)));
                    return func_body.eval_env(&new_env);
                }
            }
        }
        ...
        fn eval_letval<'a, 'b>(
            ...
            let new_env = Rc::new(Env::new(
                Some(env_rc.clone()),
                EEVal(x, e1.eval_env(env_rc)),
            ));
            return e2.eval_env(&new_env);
        }
        ...
        fn eval_letfuns<'a, 'b>(
            ...
            let mut fun_defs_map: HashMap<&'static str, &'a FunDef> = HashMap::new();

            for bind in binds.iter() {
                let (_, name, func_def) = bind;

                if fun_defs_map.contains_key(name) {
                    panic!("error: duplicated function definition: {}", name);
                }

                fun_defs_map.insert(name, func_def);
            }

            let fun_defs_list: List<(&'static str, &'a FunDef)> = fun_defs_map
                .iter()
                .fold(List::new(), |list, (name, func_def)| {
                    List::cons((name, func_def), list)
                });

            let new_env_rc = Rc::new(Env::new(Some(env_rc.clone()), EEFuns(fun_defs_list)));
            e.eval_env(&new_env_rc)
        }
